	...
	<div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
			...
			<div class=" bg-white shadow-lg rounded-sm border border-slate-200">
				<header class="px-5 py-4 border-b border-slate-100">
						<h2 class="font-semibold text-slate-800">Agenda Kegiatan</h2>
				</header>
				<div class="p-3">
								...
								<table class="table-auto w-full">
										...
										<thead class="text-xs lg:text-sm uppercase text-slate-400 bg-slate-50 rounded-sm">
												<tr>
													...
													<th class="p-2 w-36 lg:w-52 text-center">
															<div class="font-bold text-center">Tanggal</div>
													</th>
													<th class="p-2 w-40 lg:52">
															<div class="font-bold">Koordinator</div>
													</th>
													<th class="p-2">
														<div class="font-bold text-center">Tempat</div>
													</th>
										...
										<tbody class="text-sm lg:text-md font-medium divide-y divide-slate-100">
												...
												<tr>
													...
													<td class="p-2 pl-4 w-36 lg:w-52 text-center" >
														<div >{{ $item->label }}</div>
													</td>
													<td class="p-2 w-40 lg:52">
														<div class="">{{ $item->koor }}</div>
													</td>
													<td class="p-2">
														<div class="">{{ $item->locate }}</div>
													</td>
													<td class="p-2 pl-4 w-44 lg:w-60" >
														<div class="flex gap-3 justify-center items-center">
															<form action="{{ route('agenda.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Apakah Anda Yakin ?');">
																@csrf
																@method('delete')
																<button type="submit" class="btn bg-rose-500 hover:bg-rose-600 text-white">Delete</button>
															</form>
															@include('components.modal-edit-agenda')
														</div>
												</td>
											</tr>
												@endforeach
